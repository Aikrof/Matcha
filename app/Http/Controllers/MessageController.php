...
class MessageController extends Controller
{
    ...
    protected static function checkUserOnline($id)
    {
    	$user = User::find($id);

    	if (Carbon::now()->format('Y-m-d H') !==
    		Carbon::createFromFormat('Y-m-d H:i:s', $user->online)->format('Y-m-d H')){
    		
    		return (false);
    	}
    	
    	$now_min = (int)Carbon::now()->format('i');
    	$user_online_min = (int)Carbon::createFromFormat('Y-m-d H:i:s', $user->online)->format('i');

    	return (($now_min - $user_online_min > 5) ? false : true);
    }
}
